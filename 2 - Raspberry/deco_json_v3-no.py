...
def cad_proc(cad):
    ...
    try:
        ...
        if ((int(sensor_) == 1) and (int(value_) <= 20)): #Defino el rango de mis datos no deseados.Si hay una alarma inserto un 1
            send_mysql(str(sensor),str(value), "1")
            #query = "Insert into data (idsensor,data,alarma) VALUES (" + sensor_  + "," + value_ + "," + "1" + ");"
        
        elif ((int(sensor_) == 2) and (int(value_) >= 80)):
            send_mysql(str(sensor),str(value), "1")
            #query = "Insert into data (idsensor,data,alarma) VALUES (" + sensor_  + "," + value_ + "," + "1" + ");"
...
def send_mysql(sensor_,value_, alarma_="0"): # Credenciales de mi base de datos, para saber donde llevo la información.
